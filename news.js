    ...
    xhr.onreadystatechange = function() {
        if (xhr.readyState == 4 && (xhr.status == 200 || xhr.status == 0)) {
            ...
            function iteration_fonction(value,index,array) {
                ...
                let link = document.createElement("a");  // on cree un element <a>
...
function sauver_nouvelle(elt) {
	elt.src="img/disk15.jpg"; //nouvelle image disk15.jpg
  let span_2 = elt.previousElementSibling.innerHTML; // on recupere le span qui englobe l'image
  span_2.onclick = function(){supprimer_nouvelle(this)}; // on lui associe la fonction "supprimer_nouvelle"

  var myObject = {
    title : '',
    date : '',
    url : '',
  } // créer objet myObject avec un titre, une date, un url

  let span_1 = span_2.previousElementSibling.innerHTML; // on récupère span_1
  let a = span_1.previousElementSibling.innerHTML; // on récupère <a>
  myObject.title = a.firstChild.text; // on récupère le titre
  myObject.date = span_1.firstChild.text; // on récupère la date
  myObject.url = a.href; // on récupère l'url

  //on verifie que l'objet n'existe pas deja
  if(recherche_courante_news.indexOf(myObject) === -1 ){

      recherches.push(myObject); // on ajoute l'objet dans la valeur globale

      let array_recherches_courante_news_in_json = JSON.stringify(recherche_courante_news);

      setCookie("recherche_courante_news",array_recherches_courante_news_in_json,1000);  // on ecrase le cookie existant avec l'array list modifie
  }
}


function supprimer_nouvelle(elt) {
	elt.src="img/horloge15.jpg"; //nouvelle image horloge15.jpg
  let span_2 = elt.previousElementSibling.innerHTML; //on récupère le span_2 englobant l'image
  span_2.onclick = function(){sauver_nouvelle(this)};
  ...
  let span_1 = span_2.previousElementSibling.innerHTML; // on récupère span_1
  let a = span_1.previousElementSibling.innerHTML; // on récupère <a>
  myObject2.title = a.firstChild.text; // on récupère le titre
  myObject2.date = span_1.firstChild.text; // on récupère la date
  myObject2.url = a.href; // on récupère l'url

  let index = recherche_courante_news.indexOf(myObject2); // on recupere son index dans le arraylist


  recherche_courante_news.splice(index,1); // on enleve l'element de la liste

  let array_recherches_courante_news_in_json = JSON.stringify(recherches);

  setCookie("recherche_courante_news",array_recherches_courante_news_in_json,1000);
